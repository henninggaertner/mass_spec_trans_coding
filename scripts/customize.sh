...
function docker_build() {
    ...
    # check_error runs the subsequent command, and stops the running script if it errors
    # if your Dockerfile does not support changing the base image remove the --build-arg BASE_IMAGE=${BASE_IMAGE}
    check_error docker build -t "${DOCKER_TAG}" --build-arg BASE_IMAGE="${BASE_IMAGE}" .
}
